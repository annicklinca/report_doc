...
const createTable = (title, data, headers) => {
            ...
            return new TableRow({
                children: [
                    ...
                    ...headers.slice(2).map(header => (
                        new TableCell({ children: [new Paragraph(String(rowData[header] || 0))] })
                    )),
    ...
    headers.forEach(header => {
        table.getCell(0, headers.indexOf(header)).children[0].children[0].bold = true;
    });
...
const PrintDoc = () => {
    const createDocument = () => {
        ...
        const table1 = createTable("Prevalent Crimes", table1Data, ["S/N", "Crimes", "Central", "East", "North", "South", "West"]);
        const table2 = createTable("Prevalent Incidents", table2Data, ["S/N", "Incidents", "Category A", "Category B", "Category C"]);
        const table3 = createTable("Number of deaths/injuries caused by traffic accidents", table3Data, ["S/N", "Location", "Fatalities", "Injuries"]);
        const table4 = createTable("Another Table", table4Data, ["Header1", "Header2", "Header3"]);
        const table5 = createTable("Yet Another Table", table5Data, ["HeaderA", "HeaderB", "HeaderC", "HeaderD"]);
